...
switch ($_SERVER['REQUEST_METHOD']) {
	case "GET":
		...
		if($result[0] == "OK" && $result_name[0] =="OK" && $result_network[0] =="OK"){
			...
			$name = byte2str($result_name[1], 0, 64);

			$encoding = $result[1][OFF_VideoConfig+129];
			$x = byte2int($result[1], OFF_VideoConfig+130);
			$y = byte2int($result[1], OFF_VideoConfig+134);
			$framlate= byte2int($result[1], OFF_VideoConfig+139);
			$bitrate = byte2int($result[1], OFF_VideoConfig+147);

			$govLength = byte2int($result[1], OFF_VideoConfig+159);
			$codecProfile = $result[1][OFF_VideoConfig+163];
			...
			$xw->endElement();
			$xw->writeElement ('streamURI', "rtsp://$ipAddress[$dhcp_static]/profile1/media.smp");
			$xw->endElement();
