...
switch ($_SERVER['REQUEST_METHOD']) {
	case "GET":
		...
		if($result[0] == "OK" && $result_name[0] =="OK" && $result_network[0] =="OK"){
			...
			$name = byte2str($result_name[1], (708*4), 64);

			$encoding = $result[1][OFF_VideoConfig+129+(298*4)];
			$x = byte2int($result[1], OFF_VideoConfig+130+(298*4));
			$y = byte2int($result[1], OFF_VideoConfig+134+(298*4));
			$framlate= byte2int($result[1], OFF_VideoConfig+139+(298*4));
			$bitrate = byte2int($result[1], OFF_VideoConfig+147+(298*4));

			$govLength = byte2int($result[1], OFF_VideoConfig+159+(298*4));
			$codecProfile = $result[1][OFF_VideoConfig+163+(298*4)];
			...
			$xw->endElement();
			$xw->writeElement ('streamURI', "rtsp://$ipAddress[$dhcp_static]/profile5/media.smp");
			$xw->endElement();
