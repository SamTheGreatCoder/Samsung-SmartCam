...
switch ($_SERVER['REQUEST_METHOD']) {
	case "GET":
		...
		if($result[0] == "OK" && $result_name[0] =="OK" && $result_network[0] =="OK"){
			...
			$name = byte2str($result_name[1], (708*3), 64);

			$encoding = $result[1][OFF_VideoConfig+129+(298*3)];
			$x = byte2int($result[1], OFF_VideoConfig+130+(298*3));
			$y = byte2int($result[1], OFF_VideoConfig+134+(298*3));
			$framlate= byte2int($result[1], OFF_VideoConfig+139+(298*3));
			$bitrate = byte2int($result[1], OFF_VideoConfig+147+(298*3));

			$govLength = byte2int($result[1], OFF_VideoConfig+159+(298*3));
			$codecProfile = $result[1][OFF_VideoConfig+163+(298*3)];
			...
			$xw->endElement();
			$xw->writeElement ('streamURI', "rtsp://$ipAddress[$dhcp_static]/profile4/media.smp");
			$xw->endElement();
