...
switch ($_SERVER['REQUEST_METHOD']) {
	case "GET":
		...
		if($result[0] == "OK" && $result_name[0] =="OK" && $result_network[0] =="OK"){
			...
			$xw->startElement ('AVProfiles');

			for ($i = 0, $j = 0, $k = 0; $i < 6; $i++,$j+=298,$k+=708) {
				$name = byte2str($result_name[1], 0+$k, 64);
				
				$encoding = $result[1][OFF_VideoConfig+129+$j];
				$x = byte2int($result[1], OFF_VideoConfig+130+$j);
				$y = byte2int($result[1], OFF_VideoConfig+134+$j);
				$framlate= byte2int($result[1], OFF_VideoConfig+139+$j);
				$bitrate = byte2int($result[1], OFF_VideoConfig+147+$j);

				$govLength = byte2int($result[1], OFF_VideoConfig+159+$j);
				$codecProfile = $result[1][OFF_VideoConfig+163+$j];
				...
				$xw->endElement();
				$xw->writeElement ('streamURI', "rtsp://$ipAddress[$dhcp_static]/profile".($i+1)."/media.smp");
				$xw->endElement();
			}
			$xw->endElement();
			$xw->endDocument();
