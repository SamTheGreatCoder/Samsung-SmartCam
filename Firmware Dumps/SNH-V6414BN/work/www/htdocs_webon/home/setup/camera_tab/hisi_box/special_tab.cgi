...
function MakeSpecialXml()
{
	...
	<?php } ?>  	
  	...
  	<?php } ?>
  	msg += "<LDC>" + $("#ldc").val() + "</LDC>";
  	msg += "<LDCLevel>" + $("#ldc_level").val() + "</LDCLevel>";
  	msg += "</ImageSpecial>";
...
?>
	if($("#ldc").val() == 0){		//Off
		$("#ldc_level").attr("disabled", "disabled");
	}else{
		$("#ldc_level").removeAttr("disabled");
	}
	SetTimeOut();
}
...
<?php } ?>
	<tr>
		<th class="top_txt"><?php echo "LDC"; ?></th>
		<td colspan="3">
			<select id="ldc"	class="selectbox_style_wid230" onchange="OnChangeSpecial()">
				<option value="0" <?php if ($imageSpecial->dataInfo["LDC"]["value"] == 0) echo "selected=\"selected\""; ?>><?php echo $t_off[$GLOBALS["LANGINDEX"]]; ?></option>
				<option value="1" <?php if ($imageSpecial->dataInfo["LDC"]["value"] == 1) echo "selected=\"selected\""; ?>><?php echo $t_on[$GLOBALS["LANGINDEX"]]; ?></option>
			</select>
		</td>
	</tr>	
	<tr>
		<th class="top_txt"><?php echo "LDC Level"; ?></th>
		<td colspan="3">
			<select id="ldc_level" class="selectbox_style_wid230" onchange="OnChangeSpecial()">
			<?php 
				$max_ldc = 5;
				for($i=1; $i <= $max_ldc; $i++){ ?>
				<option value="<?php echo $i ?>" <?php if ($imageSpecial->dataInfo["LDCLevel"]["value"] == $i) echo "selected=\"selected\""; ?>><?php echo $i ?></option>
			<?php } ?>			
			</select>
		</td>
	</tr>
