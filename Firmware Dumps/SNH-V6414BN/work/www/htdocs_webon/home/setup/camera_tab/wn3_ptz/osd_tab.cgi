...
{
<?php 
  if ($imageOSD->dataInfo["DispDate"]["value"] == 1) { 
?>
	  $("#time_display").prop('checked', true);
<?php
  }
?>
	$("#date_notation").val(<?php echo $imageOSD->dataInfo["DateFormat"]["value"]; ?>);
	$("#time_pos_x").slider("option","value",<?php echo $imageOSD->dataInfo["DatePosX"]["value"]; ?>);
	$("#time_pos_y").slider("option","value",<?php echo $imageOSD->dataInfo["DatePosY"]["value"]; ?>);
<?php 
  if ($imageOSD->dataInfo["DispWeekday"]["value"] == 1) { 
?>
	$("#weekday_display").prop('checked', true);
<?php
  }
?>

	$("#osd_size").val(<?php echo $imageOSD->dataInfo["OSDSize"]["value"]; ?>);
	
<?php 
	if ($imageSpecial->dataInfo["PTZPositionDisplay"]["value"] == 1) { 
?>
		$("#ptzposition_display").prop('checked', true);
<?php
	}
	
	if ($imageOSD->dataInfo["PresetNameDisplay"]["value"] == 1) { 
?>
		$("#presetname_display").prop('checked', true);
<?php
	}
	
	if ($imageOSD->dataInfo["CamIDMode"]["value"] == 1) { 
?>
		$("#cameraid_display").prop('checked', true);
<?php
	}
?>
}
...
function ParsingXML_ImageOSD(xmlData)
{
	...
	$("#osd_size").val(xmlData.find('OSDSize').text());
...
<table class="data_write mgt10 mgl-20 pdl15" style="width:670px;">
	...
	<tr>
		...
		<td colspan="5" style="padding-left:0px;">
			...
			?>
				$("#camera_title").attr('maxlength', <?php echo $maxlength; ?>);
			</script>
		</td>
