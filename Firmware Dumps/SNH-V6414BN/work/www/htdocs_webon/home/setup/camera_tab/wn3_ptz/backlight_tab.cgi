...
function OnChangeBlcMode()
{
	if($("#blc_mode").val() == 1) { //BLC
		...
		//$("#aflk").removeAttr("disabled");				//exposure_tab.cgi
	}
		...
		//$("#aflk").attr("disabled","disabled");				////exposure_tab.cgi
	}
	else { //OFF
		ChangeBlcMenu('hide');
		$("#hlc_level").attr("disabled", "disabled");
	...
	if(currentCameraSetupType == 'global' && !isOnloading){ 
		if(oldBlc_mode == 3){
			<?php if(GetBrowserName() == 'IE' && ieversion() <= 7) { ?>
				SetEnableAllShutter();				//exposure_tab.cgi
			<?php } ?>
			if (SENSOR_MODE == 0){				// 60fps
				$("#shutter_speed_min").val(10);			// 1/60
			}
			else if(SENSOR_MODE == 1){
				$("#shutter_speed_min").val(9);			// 1/50
			}
			else if(SENSOR_MODE == 2){
				$("#shutter_speed_min").val(8);			// 1/30
			}
			else if(SENSOR_MODE == 3){
				$("#shutter_speed_min").val(7);			// 1/25
			}
			...
			if($("#blc_mode").val() == 3){
				...
				else if(SENSOR_MODE == 3){ 	// 25fps
					$("#shutter_speed_min").val(9);	// 1/50
				}
				$("#shutter_speed_max").val(16); 	// 1/240				
				OnChangeShutter();
			}
		}
	}	
...
?>				
	<div id="blcbox" style="display=none;width:320px;position:absolute;top:-450px;left:345px;">
<?php 
	} 
	else { 
		if ($GLOBALS['M_SUPPORT_PTZ']) {
?>
			<div id="blcbox" style="display=none;width:320px;position:absolute;top:-355px;left:360px;">
	<?php
		}
		else {
	?>
			<div id="blcbox" style="display=none;width:320px;position:absolute;top:-205px;left:360px;">
	<?php
		}
	} 
?>
