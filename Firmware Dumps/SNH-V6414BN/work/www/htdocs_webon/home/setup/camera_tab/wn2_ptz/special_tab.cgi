...
<?php
	$sliderlist = array( "color_level"=>array(0,100,$imageSpecial->dataInfo["ColorLevel"]["value"]),
...
function MakeSpecialXml()
{
	...
	<?php if($GLOBALS['M_SUPPORT_IR']) { ?> 
		if(currentCameraSetupType == 'global'){
			if(parseInt($("#ir_mode").val(),10) == 0){
...
{	
	...
	$("#shutter_speed").val(xmlData.find('ShutterSpeed').text());
<?php
	if ($GLOBALS['M_SUPPORT_DIS']) {
?>
...
function SetIrModeDependence(ptzIrMode){
	if(ptzIrMode == 0){		// off 
		if(!isOnloading){
			$('#day_night').children(":eq(2)").html("<option value=\"2\"><?php echo $t_auto[$GLOBALS['LANGINDEX']]; ?></option>");
			ChangeOption('day_night', 2, 'Enable');		//Auto
			$('#day_night').val(2);		// auto	
			$("#dwell_time").removeAttr("disabled");
			$("#duration").removeAttr("disabled");
	...
	}else if(ptzIrMode == 2){		 // Day&Night
		$('#day_night').children(":eq(2)").html("<option value=\"2\"><?php echo $t_auto[$GLOBALS['LANGINDEX']]; ?></option>");
		$('#day_night').val(2); 
		$("#dwell_time").removeAttr("disabled");
		$("#duration").removeAttr("disabled");
	}else{
		$('#day_night').children(":eq(2)").html("<option value=\"2\">---</option>");
		$('#day_night').val(2);
		$("#dwell_time").attr("disabled","disabled");
		$("#duration").attr("disabled","disabled");
	}
	...
	if(ptzIrMode != 0){
		$('#day_night').attr("disabled","disabled");
	}else{
		$('#day_night').removeAttr("disabled");
	}
