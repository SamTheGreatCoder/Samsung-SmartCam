...
<table class="data_write mgt10 mgl-20 pdl15" style="width:670px;">
	...
	<tbody>
		<tr>
			...
			<td>
				<label><input type="radio" name="ssdr_mode" id="ssdr_mode_enable" value="1" onClick="OnClickSsdrMode()" style="margin-left:<?php if (GetBrowserName()=='IE') {echo '-4';} else { echo '-1'; } ?>px" /><?php echo $t_on[$GLOBALS["LANGINDEX"]]; ?></label>
				<label><input type="radio" name="ssdr_mode" id="ssdr_mode_disable" value="0" onClick="OnClickSsdrMode()" /><?php echo $t_off[$GLOBALS["LANGINDEX"]]; ?></label>
			</td>
		</tr>
		<tr>
