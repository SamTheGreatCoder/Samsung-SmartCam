		...
		if(currentCameraSetupType == 'global') {
	 	...
	 	if(minValue >= 0 && minValue <= 10){		//x30 ~ x2
			...
			<?php if($GLOBALS['M_SUPPORT_IR']) { ?>
				var ir_mode = PTZIR_MODE;
				if(currentCameraSetupType == 'global') {
					ir_mode = parseInt($("#ir_mode").val(),10);
				}
				if(ir_mode == 2){
					if($('#agc_mode').val() == 0 || $('#agc_mode').val() == 4){ //Off, Manual
						$('#agc_mode').val(3);			//High
...
function ParsingXML_ImageExposure(xmlData)
{
	...
	if (currentCameraSetupType == 'global') {
		shutter_speed_min = xmlData.find('LongShutterSpeed').text();
		shutter_speed_max = xmlData.find('ShortShutterSpeed').text();
