...
<?php
	$sliderlist = array( "red_gain"=>array(0,255,$imageWhiteBalance->dataInfo["Red"]["value"]),
								"blue_gain"=>array(0,255,$imageWhiteBalance->dataInfo["Blue"]["value"]));
...
function ParsingXML_ImageWhiteBalance(xmlData)
{
	if (currentCameraSetupType == 'global') {
		...
		$('#wb_mode').append(
			...
			<option value=\"6\"><?php echo 'SODIUM'; ?></option>"
		);
		$('#awc_btn').show();
	}
	else {
		...
		<tr>
			...
			<td colspan="3">
				<select id="wb_mode"  class="selectbox_style_wid230" onchange="OnChangeWhitebalnaceMode()">
					...
					<option value="6" <?php if ($imageWhiteBalance->dataInfo["Mode"]["value"] == 6) echo "selected=\"selected\""; ?>><?php echo "SODIUM"; ?></option>
				</select>
			</td>
			<td>	
