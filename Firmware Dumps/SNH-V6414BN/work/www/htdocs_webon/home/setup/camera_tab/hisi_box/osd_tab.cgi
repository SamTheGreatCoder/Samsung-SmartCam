...
<?php
	...
	if ($GLOBALS["LANGINDEX"] == 2) {		// Language is "Chinese"
		if ($GLOBALS['M_MAX_RESOLUTION'] == MAX_1_MEGA){
			if($classInstances['Information']->dataInfo["Rotate"]["value"] == 0 || $classInstances['Information']->dataInfo["Rotate"]["value"] == 2) {
				$max_camtitle_pos_x = 40;
				$max_camtitle_pos_y = 24;
				
				$osd_offset_array['x'] = array(0, 13, 0);
				$osd_offset_array['y'] = array(0, 9, 0);
			}else{
				$max_camtitle_pos_x = 30;
				$max_camtitle_pos_y = 32;
				
				$osd_offset_array['x'] = array(0, 9, 0);
				$osd_offset_array['y'] = array(0, 13, 0);
			}
			...
			if($classInstances['Information']->dataInfo["Rotate"]["value"] == 0 || $classInstances['Information']->dataInfo["Rotate"]["value"] == 2) {
				...
				$max_camtitle_pos_x = 30;
				$max_camtitle_pos_y = 40;

				$osd_offset_array['x'] = array(0, 6, 11); 
				$osd_offset_array['y'] = array(0, 16, 28);
			}
		}
		...
		if ($GLOBALS['M_MAX_RESOLUTION'] == MAX_1_MEGA){
			if($classInstances['Information']->dataInfo["Rotate"]["value"] == 0 || $classInstances['Information']->dataInfo["Rotate"]["value"] == 2) {
				...
				$osd_offset_array['x'] = array(0, 8, 0);
				$osd_offset_array['y'] = array(0, 19, 0);
			}
		}else if ($GLOBALS['M_MAX_RESOLUTION'] == MAX_3_MEGA && $GLOBALS['MEGAMODE'] == 3) {
			...
			if($classInstances['Information']->dataInfo["Rotate"]["value"] == 0 || $classInstances['Information']->dataInfo["Rotate"]["value"] == 2) {
				...
				$osd_offset_array['x'] = array(0, 8, 12);
				$osd_offset_array['y'] = array(0, 24, 32);
			}
...
function OnChangeTitleTimePos(mode)
{
	...
	valX = valX - OSD_OFFSET_X[0];
	valY = valY - OSD_OFFSET_Y[0];
	...
	switch(changeMode){
		...
		default : {
			<?php if ($GLOBALS['M_ISP_TYPE'] == ISP_TYPE_HISILICON){ ?>
				$("#camera_title").attr("maxlength", 15);
			<?php }else{ ?>
