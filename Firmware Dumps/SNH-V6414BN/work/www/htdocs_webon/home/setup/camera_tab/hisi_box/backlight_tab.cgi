...
<?php
	if($classInstances['Information']->dataInfo["Rotate"]["value"] == 1 || $classInstances['Information']->dataInfo["Rotate"]["value"] == 3){
		$minBlcHeightLength = 30;
		$minBlcWidthLength = 40;
	}else{
...
{	
	var rotate = GetRotateStatus();
	if(rotate == 1 || rotate ==3){
		var MIN_BLC_HEIGHT_LENGTH = 30;
		var MIN_BLC_WIDTH_LENGTH = 40;
	}else{
	...
	if(direction == "blc_top") { 
		...
		if(value + MIN_BLC_HEIGHT_LENGTH >= MIN_BLC_HEIGHT_LENGTH){
			$("#blc_bottom").slider("option", "limitMin", value + MIN_BLC_HEIGHT_LENGTH);
		}
		else {
			$("#blc_bottom").slider("option", "limitMin", MIN_BLC_HEIGHT_LENGTH);
		}
		$("#blc_bottom").slider("option", "value", parseInt($("#blc_bottom_value").text(), 10));
	} 
	else if(direction == "blc_bottom") {
		if(value - MIN_BLC_HEIGHT_LENGTH <= 100 - MIN_BLC_HEIGHT_LENGTH ){
			$("#blc_top").slider("option", "limitMax", value - MIN_BLC_HEIGHT_LENGTH);
		}
		else {
			$("#blc_top").slider("option", "limitMax", 100 - MIN_BLC_HEIGHT_LENGTH);
		}
		...
		if(value + MIN_BLC_WIDTH_LENGTH >= MIN_BLC_WIDTH_LENGTH){			
			$("#blc_right").slider("option", "limitMin", value + MIN_BLC_WIDTH_LENGTH);
		}
		else {
			$("#blc_right").slider("option", "limitMin", MIN_BLC_WIDTH_LENGTH);
		}
		$("#blc_right").slider("option", "value", parseInt($("#blc_right_value").text(), 10));
	} 
	else {
		if(value - MIN_BLC_WIDTH_LENGTH <= 100 -  MIN_BLC_WIDTH_LENGTH){
			$("#blc_left").slider("option", "limitMax", value - MIN_BLC_WIDTH_LENGTH);
		}
		else {
			$("#blc_left").slider("option", "limitMax", 100 -  MIN_BLC_WIDTH_LENGTH);
		}
		if(value - MAX_BLC_LENGTH > 0) {
